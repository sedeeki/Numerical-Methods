...
from  Bisection_Newton_QuasiNewton import *
...
def Trapezoid_adap(f, a, b, tol=1e-5): 
    ...
    while (breakLoop == False):
      if (n == 1):
        prevIteration = 0
        currIteration = trapezoidal(f,a,b,n)
      else:
        prevIteration = currIteration
        currIteration = trapezoidal(f,a,b,n)

      difference = abs(currIteration - prevIteration)
      if (difference < tol and n > 1):
        breakLoop = True
      k = k + 1
      n = 2**k


    integral_value = currIteration
    number_of_panels = n
    return integral_value, number_of_panels
